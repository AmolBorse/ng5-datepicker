...
import { IMyDate, IMyDateRange, IMyMonth, IMyCalendarDay, IMyCalendarMonth, IMyCalendarYear, IMyWeek, IMyDayLabels, IMyMonthLabels, IMyOptions, IMyInputFieldChanged, IMyCalendarViewChanged, IMyInputFocusBlur, IMyMarkedDates, IMyMarkedDate, IMyDateFormat } from "./interfaces/index";
...
export class MyDatePicker implements OnChanges, ControlValueAccessor {
    ...
    emitDateChanged(date: IMyDate): void {
        if (this.utilService.isInitializedDate(date)) {
            let dateModel: IMyDate = this.getDateModel(date);
    ...
    getDateModel(date: IMyDate): IMyDate {
